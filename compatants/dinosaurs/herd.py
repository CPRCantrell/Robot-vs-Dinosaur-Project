...
class Herd:
    ...
    def dino_details(self):
        ...
        print(f"3) Pterodactyl:\n\t-Base evasiveness is {int((ds['Pterodactyl']['mods']['base evasion'])*100)}%\n\t-The Evade move increases to {int((ds['Pterodactyl']['mods']['evade move'])*100)}% for an extra {'turn' if ds['Pterodactyl']['mods']['evade turns'] == 1 else str(ds['Pterodactyl']['mods']['evade turns'],' turns')}\n\t-Has {str(ds['Pterodactyl']['mods']['health'])[1:]} reduced health")
